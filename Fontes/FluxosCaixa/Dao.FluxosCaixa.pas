...
type
  TDaoFluxosCaixa = class(TInterfacedObject, IDaoRetornarDados, IDaoAtulizarDados<TModelFluxosCaixa>)
  private
      ...
      FdbCommand.ExecutarSQL
         (
          ...
          'values (' + QuotedStr(FormatDateTime('dd.mm.yyyy', pFluxosCaixa.Data)) + ', ' + QuotedStr(FormatDateTime('hh:mm:ss', pFluxosCaixa.Hora)) +' ,' + sLineBreak +
                     pFluxosCaixa.IdPlanoConta.ToString + ', ' + pFluxosCaixa.DescricaoLanc.QuotedString +', '+ sLineBreak +
                     pFluxosCaixa.Valor.ToString.Replace(',', '.') +', ' + pFluxosCaixa.IdEmpresa.ToString
         );
   except on e : Exception do
       begin
           raise Exception.Create(e.Message);
